...
class ThreadPool:
    def __init__(self, num_threads, file_lock, barrier):
        self.threads = [ClientHandlerThread(i, file_lock, barrier) for i in range(num_threads)]

    def assign_connection(self, client_socket, ip):
        thread_index = hash(ip) % len(self.threads)
        self.threads[thread_index].assign_connection(client_socket, ip)
...
class ClientHandlerThread(threading.Thread):
    ...
    def handle_client(self, client_socket, ip):
        try:
            ...
            if isLastOne:
                faltantes = self.barrier.n_waiting - 1
                logging.info(f"action: ultima_apuesta_recibida | agencia_id: {clientID} | agency_ip: {ip} | faltantes: {faltantes}")
